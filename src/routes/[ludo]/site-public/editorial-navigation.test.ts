...
describe('navigation éditoriale du site public', () => {
  ...
  it('sépare la liste, la fiche et le formulaire des actualités', async () => {
    ...
    expect(detail).toContain('Supprimer définitivement')
    expect(detail).not.toContain('EditorialAssetsEditor')
    expect(detailServer).toContain('delete: async')
    expect(detailServer).not.toContain("export { actions } from '../+page.server.js'")
    ...
    expect(form).toContain('Visible sur le site')
    expect(form).toContain('Images et document')
    expect(form).toContain('attachmentFile')
    expect(form).not.toContain('Options avancées')
