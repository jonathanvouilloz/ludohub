  ...
  {:else}<div class="list">
      {#each data.documents as item (item.id)}<article
        ...
        >
          <div class="card-head">
            <div>
              <h2><a href={selectedId === item.id ? '?' : `?item=${item.id}`}>{item.title}</a></h2>
              <p class="meta">
          ...
          <footer>
            ...
            <form
                ...
                action="?/delete"
                onsubmit={(event) => {
                  if (!confirm('Supprimer définitivement ce document et son PDF ?')) event.preventDefault()
                }}
                use:enhance={toastEnhance({ success: 'Document supprimé.' })}
              >
                <input type="hidden" name="id" value={item.id} /><input
                  type="hidden"
                  name="revision"
