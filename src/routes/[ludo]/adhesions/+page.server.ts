...
export const actions: Actions = {
  configuration: async (event) => { const { ludo, member } = await requireResponsableContext(event); const input = values(await event.request.formData()); const annualFeeCents = chfToCents(input.annualFeeChf); if (annualFeeCents === null) return fail(400, { error: 'Le tarif annuel doit être indiqué en francs suisses, par exemple 30.00.' }); return wrap(() => updateFamilyForm(ludo.id, member.id, { ...input, revision: Number(input.revision), maxMembers: Number(input.maxMembers), retentionDays: Number(input.retentionDays), annualFeeCents, enabled: input.enabled === 'on', allowsTwint: input.allowsTwint === 'on', allowsCash: input.allowsCash === 'on' })) },
