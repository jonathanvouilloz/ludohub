...
<main class="family-page">
  ...
  {#if config}
    <section class="panel configuration" aria-labelledby="configuration-title">
      ...
      <form method="POST" action="?/configuration" class="configuration-form">
        ...
        <div class="input-grid">
          <div class="field"><Label for="family-fee">Tarif annuel (CHF)</Label><div class="amount-input"><Input id="family-fee" name="annualFeeChf" type="number" min="0" step="0.01" value={annualFee} required /><span aria-hidden="true">CHF</span></div><p class="hint">Par exemple, 30.00.</p></div>
          <div class="field"><Label for="family-max-members">Nombre de membres maximum</Label><Input id="family-max-members" name="maxMembers" type="number" min="1" max="50" value={config.maxMembers} required /></div>
    ...
    <section class="panel publication-panel" aria-labelledby="publication-title">
      <div><h2 id="publication-title">Mettre à jour le formulaire public</h2><p>Publiez une nouvelle version après avoir vérifié les réglages et les documents.</p></div>
      <form method="POST" action="?/publish"><input type="hidden" name="formId" value={config.id} /><input type="hidden" name="revision" value={config.revision} /><Button type="submit" variant="outline">Publier les modifications</Button></form>
    </section>

    <section class="documents-section" aria-labelledby="documents-title">
      <div class="section-intro"><div><h2 id="documents-title">Documents à accepter</h2><p>Chaque modification crée une nouvelle version conservée avec les demandes.</p></div><Badge variant="secondary">{documents.length} document{documents.length > 1 ? 's' : ''}</Badge></div>
...
<style>
  ...
  .input-grid { display: grid; grid-template-columns: repeat(3, minmax(0, 1fr)); gap: var(--space-4); }.amount-input { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; border: 1px solid var(--border-strong); border-radius: var(--radius-sm); overflow: hidden; }.amount-input :global(input) { border: 0; border-radius: 0; }.amount-input span { padding-inline: var(--space-3); color: var(--text-muted); font-size: var(--text-small); font-weight: var(--weight-semibold); }.hint { font-size: var(--text-small); }
  ...
  .publication-panel, .section-intro { display: flex; align-items: center; justify-content: space-between; gap: var(--space-5); }.publication-panel p { margin-top: var(--space-1); }.documents-section, .submissions-section { gap: var(--space-4); }.document-list { display: grid; gap: var(--space-3); }
