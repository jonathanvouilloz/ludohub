import { json } from '@sveltejs/kit'
import type { RequestHandler } from './$types'
import { PUBLIC_CACHE_CONTROL, publicCorsHeaders } from '$lib/server/public-http.js'
import { getPublicProfilesByLudoSlug } from '$lib/server/services/public-api.js'

function parseLimit(url: URL): number | null {
  const raw = url.searchParams.get('limit')
  if (raw === null) return 100
  const limit = Number(raw)
  return Number.isSafeInteger(limit) && limit >= 1 && limit <= 200 ? limit : null
}

export const GET: RequestHandler = async ({ params, request, url }) => {
  const cors = publicCorsHeaders(request)
  if (!cors) return json({ error: 'Origin not allowed' }, { status: 403 })
  const limit = parseLimit(url)
  if (limit === null) return json({ error: 'Invalid limit' }, { status: 400, headers: cors })
  const sectionParam = url.searchParams.get('section')?.trim() || undefined
  if (sectionParam !== undefined && sectionParam !== 'team' && sectionParam !== 'committee') {
    return json({ error: 'Invalid section' }, { status: 400, headers: cors })
  }
  const site = url.searchParams.get('site')?.trim() || undefined
  const payload = await getPublicProfilesByLudoSlug(
    params.ludo,
    sectionParam as 'team' | 'committee' | undefined,
    site,
    limit,
  )
  if (!payload) {
    cors.set('Cache-Control', 'no-store')
    return json({ error: 'Not found' }, { status: 404, headers: cors })
  }
  cors.set('Cache-Control', PUBLIC_CACHE_CONTROL)
  return json({ version: 1, data: payload }, { headers: cors })
}

export const OPTIONS: RequestHandler = async ({ request }) => {
  const cors = publicCorsHeaders(request)
  if (!cors) return new Response(null, { status: 403 })
  return new Response(null, { status: 204, headers: cors })
}
