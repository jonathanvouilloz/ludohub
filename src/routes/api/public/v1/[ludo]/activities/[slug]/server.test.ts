...
vi.mock('$lib/server/services/public-api.js', () => ({
  getPublicActivityDetailByLudoSlug: getDetail,
}))
...
describe('GET /api/public/v1/[ludo]/activities/[slug]', () => {
  it('répond 404/no-store pour un contenu privé, supprimé ou une mauvaise cible', async () => {
    getDetail.mockResolvedValueOnce(null)
    const request = new Request('https://api.test/api/public/v1/demo/activities/inconnue')
    const response = await GET({
      params: { ludo: 'demo', slug: 'inconnue' },
      request,
