...
<main class="mx-auto max-w-2xl space-y-6 p-6">
  ...
  <form class="grid gap-4" oninput={changed} onsubmit={(event) => { event.preventDefault(); void submit(event) }}>
    ...
    <select required name="gender"><option value="" disabled selected>Genre</option><option value="female">Féminin</option><option value="male">Masculin</option><option value="other">Neutre</option></select>
    ...
    <input required name="address" placeholder="Adresse" /><input required name="postalCode" placeholder="NPA" /><input required name="city" placeholder="Ville" />
    ...
    <fieldset class="space-y-3"><legend class="font-semibold">Membres de la famille</legend>
      {#each members as member, index}
        <div class="grid gap-2 rounded border p-3"><input required bind:value={member.firstName} placeholder="Prénom"/><input required bind:value={member.lastName} placeholder="Nom"/>{#if members.length > 1}<button type="button" onclick={() => removeMember(index)}>Retirer</button>{/if}</div>
      {/each}
    ...
    <input required name="consentFullName" placeholder="Signature (nom complet)" /><input required name="consentAcceptedOn" type="date" value={data.today} />
