...
vi.mock('./index.js', () => ({
  db: {
    ...
    batch: mocks.batch,
    query: {
      familyRegistrationForms: { findFirst: mocks.findFirst },
      familySubmissionReceipts: { findFirst: mocks.findFirst },
    },
  },
}))
import {
...
describe('paiement hors ligne', () => {
  it('verrouille par tenant/révision, exige processed et une méthode autorisée par la version', async () => {
    await recordFamilyPaymentAtomic({
...
describe('suppression manuelle', () => {
  it('supprime la demande et son reçu, sans écrire les statistiques de purge', async () => {
    await deleteFamilySubmissionAtomic({
      id: '10000000-0000-4000-8000-000000000001',
      ludoId: '20000000-0000-4000-8000-000000000001',
      expectedRevision: 3,
    })
    const query = sql(mocks.execute.mock.calls[0][0])
    expect(query).toContain('DELETE FROM family_registration_submissions')
    expect(query).toContain('DELETE FROM family_submission_receipts')
    expect(query).toContain('submission.ludo_id=')
    expect(query).toContain('submission.revision=')
    expect(query.indexOf('DELETE FROM family_registration_submissions')).toBeLessThan(
      query.indexOf('DELETE FROM family_submission_receipts'),
    )
    expect(query).not.toContain('family_processing_daily_stats')
    expect(query).not.toContain('purged_at')
  })
})
...
describe('purge PII et agrégat', () => {
  it('sélectionne uniquement les lignes avec ledger, agrège puis marque et supprime dans un statement atomique', async () => {
    ...
    expect(query.indexOf('INSERT INTO family_processing_daily_stats')).toBeLessThan(
      query.indexOf('UPDATE family_submission_receipts'),
    )
    expect(query.indexOf('UPDATE family_submission_receipts')).toBeLessThan(
      query.indexOf('DELETE FROM family_registration_submissions'),
    )
    expect(query).toContain("submission.status='processed'")
