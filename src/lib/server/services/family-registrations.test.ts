...
vi.mock('../db/family-registrations.js', () => ({
  ...
  listFamilySubmissionMemberRows: db.listMembers,
  ...
  deleteFamilySubmissionAtomic: db.deleteSubmission,
}))

import {
  deleteFamilySubmission,
  ensureFamilyForm,
  purgeDueFamilySubmissions,
  recordFamilyPayment,
  submitPublicFamilyMembership,
  updateFamilyForm,
...
const input = {
  ...
  members: [{ firstName: 'Charles', lastName: 'Lovelace' }],
}
const config = {
  form_id: '40000000-0000-4000-8000-000000000001',
  form_version_id: '50000000-0000-4000-8000-000000000001',
  version: 1,
  max_members: 20,
  consent_label: 'Texte validé',
  documents: [{ id: 'd', title: 'Règles', requiredAcceptance: true }],
}

beforeEach(() => {
  vi.clearAllMocks()
  getLudoBySlug.mockResolvedValue({ id: LUDO })
  listActiveSiteRows.mockResolvedValue([{ id: SITE, name: 'Centre' }])
  db.getConfig.mockResolvedValue(config)
  db.getReceipt.mockResolvedValue(undefined)
  db.insert.mockImplementation(async (value) => ({ id: value.id }))
  db.getSubmission.mockResolvedValue({ id: RECEIPT })
})
...
describe('soumission familiale idempotente', () => {
  ...
  it('rejoue après désactivation ou republication sans relire la configuration ni les sites', async () => {
    ...
    db.getReceipt.mockResolvedValue({
      receiptId: RECEIPT,
      requestFingerprint: written.fingerprint,
      submittedAt: new Date('2026-08-06T10:00:00Z'),
      purgedAt: new Date(),
    })
    db.getConfig.mockClear()
    listActiveSiteRows.mockClear()
    db.insert.mockClear()
    await expect(
      submitPublicFamilyMembership('demo', 'request-0000000001', input),
    ).resolves.toMatchObject({ receiptId: RECEIPT, created: false })
    expect(db.getConfig).not.toHaveBeenCalled()
  ...
  it('refuse la même clé avec un corps différent', async () => {
    ...
    db.getReceipt.mockResolvedValue({
      receiptId: RECEIPT,
      requestFingerprint: db.insert.mock.calls[0][0].fingerprint,
      submittedAt: new Date(),
    })
    await expect(
      submitPublicFamilyMembership('demo', 'request-0000000001', { ...input, city: 'Carouge' }),
    ).rejects.toMatchObject({ code: 'conflict' })
  })

  it.each([
    ['membre null', { ...input, members: [null] }],
    ['date impossible', { ...input, consentAcceptedOn: '2026-02-30' }],
  ])('rejette %s sans erreur interne', async (_label, invalid) => {
    await expect(
      submitPublicFamilyMembership('demo', 'request-0000000001', invalid as never),
  ...
  it.each([
    ...
    await expect(
      submitPublicFamilyMembership('demo', 'request-0000000001', { ...input, ...patch }),
    ).rejects.toMatchObject({ code: 'invalid' })
    expect(db.insert).not.toHaveBeenCalled()
  })
...
describe('gestion responsable', () => {
  ...
  it('audite une configuration sans inclure son texte', async () => {
    db.getForm.mockResolvedValue({ id: 'form', revision: 1 })
    db.updateForm.mockResolvedValue({ id: 'form', revision: 2 })
    await updateFamilyForm(LUDO, 'member', {
      revision: 1,
      title: 'Adhésion secrète',
      intro: null,
      consentLabel: 'Texte privé',
      enabled: true,
      retentionDays: 30,
      annualFeeCents: 3000,
      allowsTwint: true,
    ...
    expect(JSON.stringify(emitAuditEvent.mock.calls)).not.toMatch(/secrète|Texte privé/)
  })

  it('ne prétend pas enregistrer un paiement rejeté par les invariants SQL', async () => {
    db.payment.mockResolvedValue(undefined)
    await expect(recordFamilyPayment(RECEIPT, LUDO, 'member', 'twint', 1)).rejects.toMatchObject({
      code: 'conflict',
    })
...
describe('suppression manuelle', () => {
  it('retire la demande et audite seulement son identifiant', async () => {
    db.deleteSubmission.mockResolvedValue({ id: RECEIPT })
    await expect(deleteFamilySubmission(RECEIPT, LUDO, 'member', 2)).resolves.toEqual({
      id: RECEIPT,
    })
    expect(db.deleteSubmission).toHaveBeenCalledWith({
      id: RECEIPT,
      ludoId: LUDO,
      expectedRevision: 2,
    })
    ...
    expect(JSON.stringify(emitAuditEvent.mock.calls)).not.toMatch(/@|rue|téléphone/i)
  })

  it('n’audite pas une suppression refusée par la révision', async () => {
    db.deleteSubmission.mockResolvedValue(undefined)
    await expect(deleteFamilySubmission(RECEIPT, LUDO, 'member', 2)).rejects.toMatchObject({
      code: 'conflict',
    })
    expect(emitAuditEvent).not.toHaveBeenCalled()
  })
...
describe('job de purge borné', () => {
  it('enchaîne les lots complets et s’arrête au premier lot partiel', async () => {
    ...
    await expect(purgeDueFamilySubmissions(new Date('2026-09-01T00:00:00Z'))).resolves.toEqual({
      purged: 207,
      batches: 3,
      hasMore: false,
    })
  ...
  it('borne strictement le nombre de lots', async () => {
    ...
    await expect(purgeDueFamilySubmissions(new Date(), 10, 2)).resolves.toEqual({
      purged: 20,
      batches: 2,
      hasMore: true,
    })
